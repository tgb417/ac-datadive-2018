---
title: "AC Data Guide & Questions for Analysis"
author: "Emily Yelverton (Data Ambassador)"
output: 
  html_notebook:
    theme: united
    toc: yes
...
* `Registration_Start`, `Registration_End`, `Registration_Status`, `If_No_Registration_Why` : represent the dates the complaint went through the registration process with its IAM. However, note that not all IAMs put complaints through this stage - IFC_CAO, in particular, does not have one, so there will be no registration data in these columns for that IAM. `If_No_Registration_Why` is completely blank, so don't worry about including this column in your analysis.
* `Eligibility_Start`, `Eligibility_End`, `Eligibility_Status`, `If_No_Eligibility_Why_1`, ... `If_No_Eligibility_Why_3` : AC is particularly interested in analysis surrounding complaint eligibility. `Eligibility_Start` and `Eligibility_End`, as the names imply, contain the dates during which a complaint was in the eligibility determination phase. If a complaint is not found to be eligible, then it will have a reason listed in up to 3 of the `If_No_Eligibility_Why` columns. However, in a significant number of cases, a complaint will have no eligibility date, but no reason listed - look to the next columns to try and determine why. Sometimes this is because a complaint was immediately deemed ineligible at the outset and thus never went through the complaint mechanism. `Eligibility_Status` gives the explicit state of a complaint.
...
* `Monitoring_Start`, `Monitoring_End`, `Monitoring_Status`, `If_No_Monitoring_Why` : Generally speaking, if a complaint successfully passes through the compliance review or dispute resolution stages - meaning that a compliance report was published or an agreement between the opposed parties was reached - then the complaint enters a monitoring period during which the AC tries to verify that the aggrements are being honored. These columns contain the appropriate dates and statuses for this monitoring phase. 
...
* `ELIGIBLE` : This value will be null if a complaint is still active. If a complaint's Eligibility Status is "closed_with_outcome" and the complaint is not still active, this value will be 1; otherwise, it is 0. 
